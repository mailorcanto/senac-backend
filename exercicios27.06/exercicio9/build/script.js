...
function contador(string) {
    let contadorAnagramas = string.length;
    let contador = 1;
...
const permutacoesString = (string) => {
    if (string.length <= 2)
        return string.length === 2 ? [string, string[1] + string[0]] : [string];
    return string
        .split('')
        .reduce((contador, letra, i) => contador.concat(permutacoesString(string.slice(0, i) + string.slice(i + 1)).map((val) => letra + val)), []);
};
